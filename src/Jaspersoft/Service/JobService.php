...
class JobService extends JRSService
{
     ...
     * @return array
     */
    public function searchJobs($reportUnitURI = null, $owner = null,
                               $label = null, $example = null,
                               $startIndex = null, $rows = null,
                               $sortType = null, $ascending = null)
    {
        $result = array();
        ...
        $resp = $this->service->prepAndSend($url, array(200, 204), 'GET', null,
        ...
        foreach ($jobs->jobsummary as $job) {
            $result[] = @new JobSummary(
                $job->id, $job->label, $job->reportUnitURI, $job->version,
                $job->owner, $job->state->value, $job->state->nextFireTime,
    ...
    public function getJob($id)
    {
        ...
        $data = $this->service->prepAndSend($url, array(200), 'GET', null, true,
            'application/job+json', 'application/job+json');
    ...
    public function createJob(Job $job)
    {
        ...
        $data = $this->service->prepAndSend($url, array(201, 200), 'PUT',
            $job->toJSON(), true, 'application/job+json', 'application/job+json');
        return Job::createFromJSON(json_decode($data));
    }
    ...
    public function updateJob($job)
    {
        ...
        $data = $this->service->prepAndSend($url, array(201, 200), 'POST',
            $job->toJSON(), true, 'application/job+json', 'application/job+json');
        return Job::createFromJSON(json_decode($data));
    }
    ...
    {
        $url  = $this->service_url.'/jobs/'.$id;
        $data = $this->service->prepAndSend($url, array(200), 'DELETE', null,
            true);
        return $data;
    }
    ...
    public function pauseJob($jobsToStop = null)
    {
        ...
        return $this->service->prepAndSend($url, array(200), 'POST', $body,
    ...
    public function getCalendarNames($calendarType = null)
    {
        ...
        $url .= (!empty($calendarType)) ? Util::query_suffix(array("calendarType" => $calendarType))
                : null;
     ...
     */
    public function createOrUpdateCalendar(BaseCalendar $calendar,
                                           $calendarName, $replace = false,
