...
class ReportServiceTest extends BaseTest
{
    ...
    {
        $report = $this->rs->runReport('/reports/samples/FlashChartReport',
            'html');
    ...
    public function testRunCascadingInputReport_WithCustomOptions()
    {
        ...
        $report  = $this->rs->runReport('/reports/samples/Cascading_multi_select_report',
            'csv', null, null, $options);
        $this->assertContains('[Canada, USA]', $report);
        $this->assertContains('[CA, OR]', $report);
        $this->assertRegExp("/Customer\ parameter\:\,*Alcorn\-Miller\ Transportation\ Holdings/",
            $report);
    }
    ...
    public function testRunCascadingInputReport_CreateOptions()
    {
        ...
        $this->ros->updateReportOptions('/reports/samples/Cascading_multi_select_report',
            $options, 'USAAndMexicoReport', 'true');
        $report  = $this->rs->runReport('/reports/samples/USAAndMexicoReport',
            'csv');

        // Please note that this method works only when there are no whitespaces in the label.
        $this->ros->deleteReportOptions('/reports/samples/Cascading_multi_select_report',
            'USAAndMexicoReport');

        $this->assertContains('[Mexico, USA]', $report);
        $this->assertContains('[Guerrero, CA, OR]', $report);
        $this->assertRegExp("/Customer\ parameter\:\,*Adina\-Bohling\ Transportation\ Holdings/",
            $report);
    }
    ...
    public function testRunSalesByMonthReport()
    {
        ...
        $report  = $this->rs->runReport('/reports/samples/SalesByMonth', 'csv',
            null, null, $options);
        $this->assertRegExp("/Number\W*[0-9\s\,]*\W*List\ item\W*([0-9]+\s*)*\W*Date\W*(\"?\s*\w*\s*[0-9]{1,2}\,?\s*\w*\s*[0-9]{4}\"?)\W*Query\ item\W*sally/u",
            $report);

        $this->ros->updateReportOptions('/reports/samples/SalesByMonth',
            $options, 'SalesByMonthTestOptions', 'true');
        $savedOptions = $this->rs->getReportInputControls('/reports/samples/SalesByMonthTestOptions');

        try {
            $this->ros->deleteReportOptions('/reports/samples/SalesByMonth',
                'SalesByMonthTestOptions');
        } catch (Exception $e) {
