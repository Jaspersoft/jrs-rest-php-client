...
class RESTRequest
{
    ...
    protected function setAuth(&$curlHandle)
    {
        if ($this->username !== null && $this->password !== null) {
            if (empty($this->curl_cookiejar)) {
                $this->curl_cookiejar = tempnam(sys_get_temp_dir(),
                    "jrscookies_");
            }
    ...
    protected function handleError($statusCode, $expectedCodes, $responseBody)
    {
        if (!empty($responseBody)) {
            ...
            $exception                      = new RESTRequestException(
                (empty($errorData->message)) ? RESTRequestException::UNEXPECTED_CODE_MSG
                    : $errorData->message
            );
    ...
    public function makeRequest($url, $expectedCodes = array(200), $verb = null,
                                $reqBody = null, $returnData = false,
    ...
    public function prepAndSend($url, $expectedCodes = array(200), $verb = null,
                                $reqBody = null, $returnData = false,
                                $contentType = 'application/json',
                                $acceptType = 'application/json',
                                $headers = array())
     ...
     */
    public function multipartRequestSend($url, $expectedCode = 200,
                                         $verb = 'PUT_MP', $reqBody = null,
                                         $file = null, $returnData = false)
    {
    ...
    public function sendBinary($url, $expectedCodes = array(200), $body,
                               $contentType, $contentDisposition,
                               $contentDescription, $verb = "POST")
    {
        $this->flush();
        $this->setUrl($url);
        ...
        $this->setContentType($contentType);
        $this->headers = array('Content-Type: '.$contentType, 'Content-Disposition: '.$contentDisposition,
            'Content-Description: '.$contentDescription, 'Accept: application/json');
