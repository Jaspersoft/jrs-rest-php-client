...
class OptionsService extends JRSService
{
     ...
     */
    public function updateReportOptions($uri, $controlOptions, $label,
                                        $overwrite)
    {
        $url        = $this->service_url.'/reports'.$uri.'/options';
        $url        .= '?'.Util::query_suffix(array('label' => utf8_encode($label),
                'overwrite' => $overwrite));
        $body       = json_encode($controlOptions);
        $data       = $this->service->prepAndSend($url, array(200), 'POST',
            $body, true, 'application/json', 'application/json');
        $data_array = json_decode($data, true);
        return new ReportOptions($data_array['uri'], $data_array['id'],
            $data_array['label']);
    }
