...
<html>
  ...
  <body>
      ...
      <header id='intro'>
        ...
        <p>
			JasperSoft Corporation, &copy; 2013
		  <br>
		  <br>
...
<article id="create_permissions">
	...
	<article id="get_repository">
		...
		<p>
			You can search the repository for items in it, and get an array of Resource items back. If you provide no filter criteria, all items in the repository will be returned.
			There is a criteria object "\Jaspersoft\Service\Criteria\RepositorySearchCriteria" that can be used to filter for items. This is the only search function that uses a criteria object currently.
			In the future, all search functions will have criteria objects used to filter results.
			<br>
	...
	<article id="create_resource">
		...
		<p>
			In addition to binary resources, you can also create other types of resources in the repository, such as Folders, Data Sources, and most all Resources available in the repository. The example below
			will show how to create a new Folder on the server.
		</p>
		<pre>
		<code>
	...
	<article id="delete_resource">
		...
		<p>
			You can remove resources from the repository, either one at a time, or several at a time. Using deleteResource or deleteManyResources.
		</p>
		<pre>
		<code>
	...
	<article id="move_resource">
		<h3> Move Resource </h3>
		<p>
			Resources can be moved from one location to another within the Repository. The example below will move the folder "/ImageFolder/anchorsteam" to "/anchorsteam"
		</p>
		<pre>
		<code>
	...
	<article id="copy_resource">
		<h3> Copy Resource </h3>
		<p>
			Resources can be copied from one location to another within the Repository. The example below will copy the folder "/anchorsteam" to the new location "/ImageFolder/anchorsteam". By setting the last
			argument to true, folders which do not exist will be created when copying the file.
		</p>
		<pre>
		<code>
	...
	<article id="get_role">
		...
		<p>
			If you know the name of the role, you can request it specifically using this function. The example below will request the ROLE_USER data.
		</p>
		<pre>
		<code>
