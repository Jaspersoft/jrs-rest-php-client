...
class PermissionService extends JRSService
{
    ...
    private function batchDataToArray($json_data)
    {
        ...
        foreach ($data_array->permission as $perm) {
            $result[] = @new RepositoryPermission($perm->uri, $perm->recipient,
                $perm->mask);
        }
        return $result;
    }
     ...
     */
    public function searchRepositoryPermissions($uri,
    ...
    public function updateRepositoryPermission(RepositoryPermission $permission)
    {
        ...
        $data = $this->service->prepAndSend($url, array(200), 'PUT',
    ...
    public function updateRepositoryPermissions($uri, $permissions)
    {
        ...
        $permissions = $this->service->prepAndSend($url, array(200), 'PUT',
            $body, true, 'application/collection+json', 'application/json');
    ...
    public function createRepositoryPermissions($permissions)
    {
        ...
        $body        = json_encode(array('permission' => $permissions));
        $permissions = $this->service->prepAndSend($url, array(201), 'POST',
            $body, true, 'application/collection+json', 'application/json');
        return self::batchDataToArray($permissions);
    }
    ...
    public function createRepositoryPermission(RepositoryPermission $permission)
    {
        ...
        $response = $this->service->prepAndSend($url, array(201), 'POST', $body,
            true);
    ...
    {
        $url = $this->service_url.'/permissions'.$permission->uri.';recipient='.str_replace('/',
                '%2F', $permission->recipient);
